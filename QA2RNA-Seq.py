...
parser.add_argument('--infile', '-f', nargs='+', help="Examples: -f file1.fq file2.fq")
parser.add_argument("--barcode", "-b", help="Barcode generated data answer in Yes/No")
parser.add_argument("--barcodefile", "-bf", help="Provide barcode file in fasta format")
...
parser.add_argument("--qualityscore","-QS", help="Quality score for read filtering")
...
parser.add_argument("--precursor","-prec", help="Precursor miRNA sequence file in fasta format")
parser.add_argument("--mature","-mat", help="mature miRNA sequence file in fasta format")
...
os.system("mkdir QCS")
if os.path.exists("reads_config.txt"):
  os.remove("reads_config.txt")
  ...
  if args.seqType == "smallRNA" :
    ...
    if len(args.infile) > 1:
      ...
      for ifile in args.infile:
        ...
        os.system("fastqc -o QCS --contaminants contaminant_list.txt --adapters adapter_list.txt " + outname1 + " --threads " + args.processor+" 2>fastqc_log")
        print("\nQuality check is finished\n")
        if args.barcodefile:
          print("Barcode file is provided in fasta format")
          os.system("mkdir "+outname+"_barcode")
          lissplit.append(outname+"_barcode")
          print("Fastq files splitting according to barcode sequence")
          os.system("flexbar -r "+outname1+" -b "+args.barcodefile+" -bt ANY -n "+args.processor+" -bi 0 -z GZ -bu -t "+outname+"_barcode/"+outname)
          print("Splitting is done")
        else:
      	  print("No barcode provided")
      for k in lissplit:
        print("Merging the splitted files")
        os.system("perl CombineTobarcodefiles.pl "+k)
        print("Merging done!")
        kk = re.sub("_barcode", "", k)
        print(lissplit.index(k))
        icode=lissplit.index(k)+1
        print(icode)
        if args.adapter:
          Trim.trimm(kk+"_join", args.adapter, args.mismatch, args.qualityscore, args.processor, icode)
        elif args.adapterfile:
          Trim.trimm(kk+"_join", args.adapterfile, args.mismatch, args.qualityscore, args.processor, str(icode))
        else:
          print("Default universal adapter")
      if args.genome:
         print("genome is provided")
         miRQT.miRqt(args.genome, args.precursor, args.mature, args.processor)
      else:
         print("Please provide genome sequence file")
    else:
